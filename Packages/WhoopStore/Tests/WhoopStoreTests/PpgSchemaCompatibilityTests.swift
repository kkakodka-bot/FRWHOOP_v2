...
final class PpgSchemaCompatibilityTests: XCTestCase {
    func testCutoverDatabaseKeepsServerCacheAndLegacyPpgRows() throws {
        ...
        try dbq.write { db in
            try db.execute(sql: """
                ...
                VALUES (88, 'legacy', 123, X'0100', 14);
                INSERT INTO serverScoreCache (day, algorithmVersion, nightsJson, fetchedAt)
                VALUES ('2026-09-18', 'existing-score', '[]', 123);
                """)
        }
        try WhoopStore.makeMigrator().migrate(dbq)
        try dbq.read { db in
            XCTAssertEqual(try Int.fetchOne(db, sql: "SELECT rowid FROM ppgWaveformSample"), 88)
            XCTAssertEqual(try Int.fetchOne(db, sql: "SELECT recordIndex FROM ppgWaveformSample"), -1)
            XCTAssertEqual(try String.fetchOne(db, sql: "SELECT algorithmVersion FROM serverScoreCache"),
                           "existing-score")
    ...
    func testLegacyMigrationPreservesBytesAndRowIDsUsedByUploadCursors() throws {
        let dbq = try DatabaseQueue()
        try WhoopStore.makeMigrator().migrate(dbq, upTo: "v45-backfill-frontier")
