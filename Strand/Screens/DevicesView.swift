...
// MARK: - Devices
//
// Pair and manage the bands NARA reads from. WHOOP-FIRST: the WHOOP is the primary, fully-supported
...
struct DevicesView: View {
    ...
    var body: some View {
        ScreenScaffold(title: "Devices",
                       subtitle: "Pair and manage the bands NARA reads from.",
            ...
            if let registry = model.deviceRegistry {
                ...
                DataPendingNote(
                    ...
                    message: "NARA is opening your on-device data. Your paired bands will appear here in a moment.",
...
private struct DevicesContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.sectionSpacing) {
            ...
            ForEach(Array(activeDevices.enumerated()), id: \.element.id) { idx, device in
                ...
                DeviceCard(
                    ...
                    // Firmware version for the ACTIVE strap. It's a STABLE property (NARA can't change a
        ...
        .alert("Remove this device?",
            ...
            Text("Remove \(device.displayName)? NARA will stop connecting to it. Its recorded data is kept and you can re-add it any time.")
        }
    ...
    private var whoopFirstFooter: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: "info.circle")
                ...
                .accessibilityHidden(true)
            Text("WHOOP is NARA's primary, fully-supported band. Other heart-rate straps are an early, in-development addition: they stream live heart rate and HRV, but not WHOOP's deeper sleep and recovery data.")
...
private struct DeviceCard: View {
    ...
    /// The active strap's link dropped for a user-initiated reboot and NARA is auto-reconnecting (#166).
    ...
    private var cardContent: some View {
        StrandCard(padding: 18, tint: isActive ? StrandPalette.accent : nil) {
            VStack(alignment: .leading, spacing: NoopMetrics.cardInnerSpacing) {
                ...
                // was reset again or re-claimed in the Oura app, NARA no longer owns it) without faking a
                ...
                capabilityRow(symbol: "waveform.path.ecg", text: profile.captures,
                              tint: StrandPalette.textSecondary)
                // What NARA USES it for — the scores/screens this device drives.
                ...
                // banks no history in that state, which otherwise looks like a NARA sync bug).
    ...
    private var ouraLocalStateNote: some View {
        HStack(alignment: .top, spacing: 6) {
            Image(systemName: "info.circle")
                ...
                .frame(width: 14)
                .accessibilityHidden(true)
            Text("Paired locally. NARA owns this ring while it holds the key. If you reset it again or set it up in the Oura app, NARA no longer owns it and you would re-add it to take it over.")
                .font(StrandFont.caption)
                .foregroundStyle(StrandPalette.statusWarning)
                .fixedSize(horizontal: false, vertical: true)
        }
...
/// Honest, per-model summary of what a device captures and what NARA uses it for — shown on its card.
///
...
/// and Charge/Effort/Rest are NARA-derived scores. Verdicts are source-verified against the decode +
...
struct DeviceCapabilityProfile {
    ...
    let powers: String         // the NARA scores / screens this device drives
    ...
    static func make(for d: PairedDevice) -> DeviceCapabilityProfile {
        ...
        if d.sourceKind == .huami {
            return DeviceCapabilityProfile(
                ...
                footnote: String(localized: "Experimental: live heart rate where the band exposes it. Some bands need a pairing we can't do yet. NARA will say so honestly and never show a made-up number. No sleep, recovery, skin temp, SpO₂ or steps."))
        }
        // EXPERIMENTAL locally-adopted Oura ring (gen 3/4/5). The gen is carried on `model` ("Oura Ring
        // 3/4/5") and recovered with OuraRingGen.from(model:). NARA reads the ring's OWN raw signals + open
        ...
        // WHOOP 4.0 — NARA's primary band; no steps over BLE.
        if model.contains("4") {
            return DeviceCapabilityProfile(
    ...
    func body(content: Content) -> some View {
        content
            .alert("Forget this device?",
                ...
                Text("NARA removes this device from your list and deletes its recorded data here. You can re-pair the strap to pull its recent history back.")
            }
    }
}
...
struct DeviceCardCatalog: View {
    ...
    var body: some View {
        ScreenScaffold(title: "Devices",
                       subtitle: "What each band captures (and what NARA uses it for).",
                       topBackground: liquidScaffoldSky()) {
            VStack(spacing: NoopMetrics.gap) {
                ...
                DeviceCard(device: Self.dev("whoop-5-refused", "WHOOP", "5.0 MG",
                           ...
                           pairingHint: "NARA can see your strap but it's refusing to pair - it's likely still bonded to the official WHOOP app, or your phone is holding an old pairing. To fix it: (1) fully close the WHOOP app, (2) on a 5.0/MG, tap the band repeatedly until the LEDs flash blue (pairing mode), (3) if your strap is listed under iPhone Settings → Bluetooth, tap it and choose Forget This Device, then reconnect in NARA.",
                           onMakeActive: {}, onRename: {}, onRemove: {})
