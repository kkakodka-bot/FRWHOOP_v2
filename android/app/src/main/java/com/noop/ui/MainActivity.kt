...
object NoopPrefs {
     ...
     *  Default ON for this fork so the tile can fill when no calibrated import exists; an explicit user
     *  choice (including OFF) is always honoured. Twin of iOS `PuffinExperiment.spo2CandidateDisplayEnabled`. */
    fun spo2CandidateDisplay(context: Context): Boolean {
        val prefs = of(context)
        if (!prefs.contains(KEY_SPO2_CANDIDATE_DISPLAY)) return true
        return prefs.getBoolean(KEY_SPO2_CANDIDATE_DISPLAY, false)
    }

    /**
     * Pins the ON default for installs that never chose. Idempotent; returns whether a write happened so the
     * caller can trigger one immediate re-score. Twin of iOS `PuffinExperiment.migrateSpo2CandidateDisplayDefault`.
     */
    fun migrateSpo2CandidateDisplayDefault(context: Context): Boolean {
        val prefs = of(context)
        if (prefs.contains(KEY_SPO2_CANDIDATE_DISPLAY)) return false
        prefs.edit().putBoolean(KEY_SPO2_CANDIDATE_DISPLAY, true).apply()
        return true
    }
    ...
    fun spo2CandidateDisplayFlow(context: Context): Flow<Boolean> = callbackFlow {
        ...
        val listener = SharedPreferences.OnSharedPreferenceChangeListener { changed, key ->
            ...
            if (key == null || key == KEY_SPO2_CANDIDATE_DISPLAY) {
                trySend(spo2CandidateDisplay(context.applicationContext))
            }
        }
