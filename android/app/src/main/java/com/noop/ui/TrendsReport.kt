...
object TrendsReportRenderer {
    ...
    fun renderPdf(
        ...
        val file = File(dir, "NARA-trends-${report.start}_to_${report.end}.pdf")
    ...
    private fun drawHeader(canvas: Canvas, report: RangeReport, range: ReportRange, top: Float): Float {
        ...
        val left = MARGIN + 16f
        var ty = cardTop + 26f
        text(canvas, "NARA", left, ty, 11f, sansBold, ACCENT, letterSpacing = 0.12f)
    ...
    private fun drawFooter(canvas: Canvas, generatedOn: String) {
        ...
        val legend = "How to read this: HRV, Resting HR, Sleep duration, Respiratory rate and Skin " +
            ...
            "Strain and Stress are NARA's own on-device scores, not clinical measures - Recovery is a daily " +
        ...
        text(
            canvas,
            "Generated by NARA on $generatedOn · all on-device, no account, no cloud.",
