...
import { createManifestStore, READY_STATUSES } from '../storage/manifests.js';
import {
...
import { MAX_OBJECT_LANE_BYTES, MAX_RANGE_MS, expiresAt } from '../storage/retention.js';
import { inc } from '../observability/metrics.js';
import { decompressObject } from '../storage/rawObjectReader.js';
import { sha256Hex } from './archiveFormat.js';
 ...
 */
export function validateObjectIntent(manifest) {
  const errors = [];
  const m = manifest || {};
...
export function createPushObjects({
  ...
  const manifests = rest?.configured ? createManifestStore({ rest, now }) : null;
  ...
  return {
    get configured() {
      return Boolean(manifests && cfg?.b2KeyId && cfg?.b2ApplicationKey && cfg?.b2Bucket);
    },

    async createIntent({ userId, manifest }) {
      if (!isUuid(userId)) throw fail('unauthorized', 401);
      if (!manifests) throw fail('archive_not_configured', 503);
      const raw = await objectStore();
      ...
      await manifests.insertPending({
        ...
        expires_at: expiresAt(manifest.stream, now(), cfg),
     ...
     */
    async completeObject({ userId, objectId }) {
      if (!isUuid(userId)) throw fail('unauthorized', 401);
    ...
    /**
     * Reads the object back and hashes it. On match the row becomes `verified`; on mismatch it
     * becomes `corrupt` and is NOT readable as archive. Run out of band — it costs a full download,
     * which is exactly the cost the direct lane exists to avoid on the hot path.
     */
    async verifyObjectDigest({ objectId }) {
      if (!manifests) throw fail('archive_not_configured', 503);
      const raw = await objectStore();
      const row = await manifests.get(objectId);
      if (!row) return { ok: false, error: 'missing_manifest' };
      if (row.sha256_source === SHA_SOURCE.verified) return { ok: true, row, duplicate: true };
      ...
      /**
       * Hash the DECOMPRESSED payload, because that is what the device hashed. The digest is taken
       * over the signal rather than the wire bytes so it survives a change of compressor — hashing
       * `obj.body` here would compare a gzip frame against a payload digest and never match.
       *
       * Decompressing also makes this the stronger check: a frame that no longer inflates is caught
       * here as `unreadable` instead of sitting in the bucket as a `ready` object that only fails
       * the day someone tries to read it.
       */
      let payload;
      try {
        payload = decompressObject(obj.body, pushArchiveSpecForStream(row.object_kind).compression);
      } catch {
        inc('object_verification_failures');
        await manifests.mark(objectId, { status: 'corrupt' });
      ...
      if (row.uncompressed_bytes != null && payload.length !== Number(row.uncompressed_bytes)) {
        inc('object_verification_failures');
        await manifests.mark(objectId, { status: 'corrupt' });
        return {
          ok: false,
          error: 'uncompressed_size_mismatch',
          expected: Number(row.uncompressed_bytes),
      ...
      if (row.sha256 && actual !== row.sha256) {
        inc('object_verification_failures');
        await manifests.mark(objectId, { status: 'corrupt' });
        return { ok: false, error: 'sha256_mismatch', expected: row.sha256, actual };
      }
      const at = now().toISOString();
      const updated = await manifests.mark(objectId, {
        status: 'verified',
        sha256: actual,
        sha256_source: SHA_SOURCE.verified,
        verified_at: at,
      });
      return { ok: true, row: (Array.isArray(updated) ? updated[0] : updated) || row };
    },
  };
}
