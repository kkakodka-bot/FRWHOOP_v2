...
export const APPEND_STREAM_PROJECTIONS = {
  hrSample: {
...
export const REPLACE_STREAM_PROJECTIONS = {
  dailyMetric: {
    table: 'daily_metrics',
  ...
  sleepSession: {
    ...
    mapRow: ({ userId, deviceId, headerDeviceId, record }) => {
      ...
      const row = sleepSessionRow({ userId, deviceUuid: deviceId, session });
      row.summary = {
        ...row.summary,
  ...
  workout: {
    ...
    mapRow: ({ userId, deviceId, headerDeviceId, record }) => {
      ...
      const row = workoutSessionRow({ userId, deviceUuid: deviceId, workout });
      row.summary = {
        ...row.summary,
...
export function recordTimestamp(stream, record) {
...
export function archiveWindowFromRecords(stream, records, fallbackDate = new Date(), header = null) {
  const projection = APPEND_STREAM_PROJECTIONS[stream] || REPLACE_STREAM_PROJECTIONS[stream];
...
export function replacementKeys(stream, records, headerDeviceId) {
...
export function windowBounds(header) {
...
export function streamsForVersion(version) {
...
export function advertisedStreams(protocolVersion, enabledStreams = INGEST_ENABLED_STREAMS) {
  const allowed = streamsForVersion(protocolVersion);
  return [...enabledStreams].filter((s) => allowed.has(s)).sort();
}

export function negotiateProtocol(acceptHeader) {
...
export function parseNdjsonEntity(bytes) {
...
export function buildAck(header) {
  return {
...
export function ackMatchesBatch(ack, header) {
