-- Canonical engine ingest/read RPCs used by backend/metrics/repository.js.
-- Supersedes the engine_* definitions in 20260824120000 with:
--   * service-role pass in internal.assert_ingest_secret
--   * the full daily_metrics column set (steps, spo2, vo2max, HR stats, ...)
--   * merge semantics for jsonb blobs (extras/confidence/provenance merge,
--     chart_data replaced only when the new payload actually has data)
-- Also creates the partial unique indexes the conflict clauses rely on, for
-- fresh environments (they already exist on the production project).
