-- Gravity sleep persistence.
--
-- The sleep engine owns the current algorithm projection for one
-- user/device/physiological day. This migration adds an atomic replacement RPC
-- for that projection, wires it into engine_ingest_extras, and leaves manual or
-- user-modified sessions untouched.

begin;

-- Direct service-role PostgREST upserts must not undo a boundary correction.
create or replace function internal.preserve_user_modified_session_bounds()
returns trigger
language plpgsql
set search_path = pg_catalog
as $$
begin
  if old.user_modified then
...
drop trigger if exists sessions_preserve_user_bounds on public.sessions;
create trigger sessions_preserve_user_bounds
  before update on public.sessions
  for each row execute function internal.preserve_user_modified_session_bounds();

revoke all on function internal.preserve_user_modified_session_bounds()
...
begin
  ...
  -- Remove projections no longer produced by the algorithm. Cascading removes
  -- their sleep_details rows. Manual rows, other sources, and boundary-corrected
  ...
  with stale as (
    ...
    where s.user_id = v_user
      ...
      and public.local_calendar_date(
...
-- Retain existing extras behavior and add canonical sleep_details persistence.
...
begin
  ...
  if p_payload ? 'sleep_details' then
    v_sleep := public.engine_replace_sleep_day(p_secret, p_payload);
  end if;
  ...
  loop
    insert into public.metric_runs (
      id, user_id, period_day, algorithm, version, status, input_refs, output_refs,
...
-- RPC-only deployments authenticate as anon with INGEST_SECRET. Table access
-- remains unavailable; only these secret-checking functions are executable.
...
grant execute on function public.engine_ingest_extras(text, jsonb)
  to anon, service_role;

-- Complete the repository's existing anon-key + ingest-secret deployment path.
-- Each function performs internal.assert_ingest_secret before reading/writing.
...
revoke all on function public.engine_load_user_days(text, uuid, date, date)
  from public, anon, authenticated;
grant execute on function public.engine_ingest_upsert(text, jsonb)
  to anon, service_role;
grant execute on function public.engine_load_user_days(text, uuid, date, date)
  to anon, service_role;
