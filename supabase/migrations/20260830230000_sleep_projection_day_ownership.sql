-- Sleep projection ownership across local midnight.
--
-- A sleep/nap projection row belongs to exactly ONE physiological day: the
-- ENGINE day encoded in its external_id (sleep:<device>:<day>:<slot>). The
-- stale-session cleanup in engine_replace_sleep_day matched rows by the
-- LOCAL CALENDAR DATE of their end instant instead. Those two days disagree
-- exactly at midnight: a night attributed to day N can end at 00:00:01 on
-- local day N+1, and an afternoon episode attributed to day N can end on
-- local day N-1. The cleanup then deleted rows owned by a NEIGHBOR day's
-- projection — in production (2026-08-30) a day's finalize wrote its session
-- and the next day's finalize silently deleted it, so the projection never
-- survived and the finalizer reported persistence_missing:session forever.
--
-- This migration adds public.sleep_projection_day() (external_id day when
-- parseable, local end date otherwise) and keys the cleanup on it. Same-day
-- stale cleanup semantics are unchanged: a day's replacement still deletes
-- that day's algorithm-owned rows that the incoming projection no longer
-- produces.
