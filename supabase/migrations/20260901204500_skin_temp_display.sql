...
-- 12-minute skin temp series (~5/hour) plus skin_temp_c on snapshot/range.

alter table public.daily_physiology_series
  add column if not exists skin_temp_series jsonb not null default '[]'::jsonb;

alter table public.daily_physiology_series
  drop constraint if exists daily_physiology_series_skin_temp_size_check;

alter table public.daily_physiology_series
  add constraint daily_physiology_series_skin_temp_size_check
  check (pg_column_size(skin_temp_series) <= 65536);
